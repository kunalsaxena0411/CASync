...
export default function GstReturnPage() {
  ...
  useEffect(() => {
    ...
    const userGSTs = JSON.parse(localStorage.getItem(`gst_${email}`) || "[]");
  ...
  function handleSubmit(event) {
    ...
    const filedReturns = JSON.parse(localStorage.getItem("gstReturns") || "[]");
    filedReturns.push({
      loggedInUser,
      gstNumber: selectedGst,
      businessName: selectedRegistration?.businessName || "",
    ...
    localStorage.setItem("gstReturns", JSON.stringify(filedReturns));
      ...
      <div className="form-container">
        ...
        <form id="gstReturnForm" onSubmit={handleSubmit}>
          <div className="form-group">
            ...
            <select id="gstRegistration" required value={selectedGst} onChange={(event) => setSelectedGst(event.target.value)} disabled={registrations.length === 0}>
              <option value="">Select GST Registration</option>
              {registrations.map((gst) => (
                <option key={gst.gstNumber} value={gst.gstNumber}>
                  {`${gst.gstNumber} - ${gst.businessName}`}
                </option>
              ))}
            </select>
          </div>

          <div className="form-group"><label htmlFor="businessName">Business Name:</label><input type="text" id="businessName" readOnly value={selectedRegistration?.businessName || ""} /></div>
          <div className="form-group"><label htmlFor="state">State:</label><input type="text" id="state" readOnly value={selectedRegistration?.address || ""} /></div>
          <div className="form-group"><label htmlFor="mainPerson">Main Person:</label><input type="text" id="mainPerson" readOnly value={selectedRegistration?.mainPerson || ""} /></div>
          <div className="form-group"><label htmlFor="panNo">PAN Number:</label><input type="text" id="panNo" readOnly value={selectedRegistration?.panNo || ""} /></div>
          <div className="form-group">
          ...
          <div className="form-group">
            ...
            <input type="file" id="documents" multiple onChange={(event) => setDocuments(Array.from(event.target.files || []).slice(0, 20))} />
