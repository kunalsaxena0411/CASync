...
export default function AddPanPage() {
  ...
  async function handleSubmit(event) {
    ...
    const existingIndex = panRecords.findIndex((entry) => entry.pan === normalizedRecord.pan);
    if (existingIndex >= 0) {
      panRecords[existingIndex] = normalizedRecord;
    } else {
      panRecords.push(normalizedRecord);
    }
    localStorage.setItem("panRecords", JSON.stringify(panRecords));
